        ...
        <div class="P-4 pt-5 flex justify-center pl-6 bg-gray-700 w-5/6 rounded-md bg-opacity-25">
            ...
            @method('PUT')
                <div class="flex flex-wrap w-full uppercase text-xl justify-start text-flugreen-500">
                    <p><strong>Nueva sub categoria :</strong></p>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    id="grid-first-name" type="text" name="name"  value="{{$subCategory->name }}">
                    ...
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <div class="relative">
                            ...
                            rounded w-full py-2 px-3 text-flugreen-500  leading-tight 
                            focus:outline-none focus:bg-gray-700 focus:border-gray-500"
                            id="grid-state" name="category" >
                                <option disabled selected>..SELECCIONE..</option>
                                @foreach ($categories as $value)
                                    <option value="{{$value->id}}"{{ old('category') == $value->id ? 'selected' : '' }}{{ $subCategory->id_category == $value->id ? 'selected' : '' }}>{{ $value->name}}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    href="{{route('sub-categories.index')}}">
                        <strong>Cancelar edición</strong>
                    </a>
                </div>
            </form>
        </div>
